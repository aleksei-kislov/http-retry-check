...
Run the complete Go example from the repository root with Go 1.26 or later:
...
The module requires Go 1.26 or later; CI uses Go 1.26.6. The example creates a
proxy-free HTTP/1 client and runs all six scenarios. Start with the
[five-minute guide](docs/guides/developer-quickstart.md) to use C# instead or
to check a client from another project.
...
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for notices associated with
the source and its development dependencies.
