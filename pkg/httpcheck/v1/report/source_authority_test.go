...
func TestPublicSurfaceAndProductionDependencies(t *testing.T) {
	...
	wantVariables := map[string]string{
		"ErrInvalidReport":   "invalidReport",
		"ErrInvalidArtifact": "invalidArtifact",
	}
	seenFunctions := make(map[string]bool)
	seenTypes := make(map[string]bool)
	seenConstants := make(map[string]bool)
	seenVariables := make(map[string]string)
						...
						for _, valueName := range specification.Names {
							if !ast.IsExported(valueName.Name) {
								if current.Tok == token.VAR {
									t.Errorf("unexpected unexported package variable %s", valueName.Name)
								}
								continue
							}
							if current.Tok == token.VAR {
								wantInitializer, ok := wantVariables[valueName.Name]
								if !ok || len(specification.Names) != 1 || len(specification.Values) != 1 ||
									identifierName(specification.Type) != "error" ||
									identifierName(specification.Values[0]) != wantInitializer {
									t.Errorf("unexpected exported variable %s", valueName.Name)
									continue
								}
								seenVariables[valueName.Name] = wantInitializer
								continue
							}
							if current.Tok != token.CONST {
								t.Errorf("unexpected exported value %s", valueName.Name)
								continue
							}
	...
	if !reflect.DeepEqual(seenFunctions, wantFunctions) || !reflect.DeepEqual(seenTypes, wantTypes) ||
		!reflect.DeepEqual(seenConstants, wantConstants) || !reflect.DeepEqual(seenVariables, wantVariables) ||
		!seenPrivateErrorMethod {
		t.Fatalf("surface = functions %v, types %v, constants %v, variables %v",
			seenFunctions, seenTypes, seenConstants, seenVariables)
	}
}
...
func TestPublicSignaturesConstantsAndStructShapes(t *testing.T) {
	...
	if SchemaVersion != "http_retry_check.report.v2" ||
		...
		ErrInvalidReport != invalidReport || ErrInvalidArtifact != invalidArtifact ||
	...
	assertExactStruct(t, observationType, []expectedField{
		...
		{"AttemptLimit", uint32Type, "attempt_limit"}, {"Protocol", stringType, "protocol"},
...
func rootNamedType(t *testing.T, name string) reflect.Type {
	...
	types := []reflect.Type{
		...
		reflect.TypeOf(Observation{}).Field(16).Type,
		reflect.TypeOf(Observation{}).Field(17).Type,
	}
