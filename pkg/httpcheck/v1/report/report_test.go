...
func TestInvalidEvidenceErrorsMatchExportedSentinels(t *testing.T) {
	forged := mustReport(t, positiveResult())
	forged.ClaimCeiling = sanitationMarker
	if err := Validate(forged); !errors.Is(err, ErrInvalidReport) || errors.Is(err, ErrInvalidArtifact) {
		t.Fatalf("forged report error = %v", err)
	}

	tampered := mustArtifact(t, positiveResult())
	tampered[1].Contents[0] ^= 0xff
	if err := ValidateArtifact(tampered); !errors.Is(err, ErrInvalidArtifact) || errors.Is(err, ErrInvalidReport) {
