...
func TestLegacyVocabularyAndObservationMapping(t *testing.T) {
	...
	legacyObservation := legacysuite.Observation{
		CaptureComplete: true, AttemptCount: 3, EffectCount: 2, OverlapCount: 1,
...
func TestTypedNilAndRuntimeErrorsReturnSanitizedResults(t *testing.T) {
	var typedNil *markerDoer
	for _, doer := range []Doer{typedNil, markerDoerValue{}} {
...
func TestFixedErrorsAndExplanations(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{FindingText(FindingAttemptLimitExceeded), "The client made more than two attempts."},
...
func TestGoRejectsInvalidSemanticCorpusVectors(t *testing.T) {
	...
	if invalids.SchemaVersion != "http_retry_check.conformance_invalid.v1" ||
		invalids.Category != "semantic" || invalids.Cases == nil || len(invalids.Cases) != 64 {
	...
	if goCases != 63 || !forgedScenarioIncomplete {
		t.Fatalf("Go invalid vectors/forged scenario_incomplete = %d/%t, want 63/true",
...
func TestGoCorpusResultsDoNotLeakSanitationMarker(t *testing.T) {
	...
	for _, vector := range sanitation.Cases {
		...
		t.Run(vector.ID, func(t *testing.T) {
			...
			if !ok || vector.Target != target || vector.Operation != "inject_marker" ||
				vector.Operand != sanitation.Marker || vector.Expected != "marker_absent" ||
