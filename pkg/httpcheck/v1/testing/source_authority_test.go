...
func TestExactTestingChildSourceAndPublicSurface(t *testing.T) {
	...
	if !checkType.IsVariadic() || checkType.NumIn() != 3 ||
		checkType.In(0) != reflect.TypeOf((*testing.T)(nil)) ||
		checkType.In(1) != reflect.TypeOf((*httpcheck.Doer)(nil)).Elem() ||
		checkType.In(2) != reflect.TypeOf([]httpcheck.Option{}) || checkType.NumOut() != 0 {
...
func TestCheckLifecycleAndDiagnosticsStayExact(t *testing.T) {
	...
	want := map[string]int{
		"httpcheck.Run": 1, "httpcheck.Validate": 1, "t.Helper": 2, "t.Context": 1,
		"t.Fatal": 4, "t.Error": 1, "t.Log": 1,
	}
