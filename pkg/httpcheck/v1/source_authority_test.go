...
func TestProductionSourceAndPublicSurface(t *testing.T) {
	...
	for _, required := range []string{"doc.go", "evaluate.go", "explain.go", "run.go", "types.go"} {
	...
	wantTopLevelFunctions := map[string]bool{
		...
		"AssessmentText": true, "FindingText": true,
	}
...
func TestFunctionAndStructSignaturesMatchExpectedAPI(t *testing.T) {
	...
	if runType.NumIn() != 2 || runType.In(0) != reflect.TypeOf((*context.Context)(nil)).Elem() ||
		runType.In(1) != doerType || runType.NumOut() != 2 ||
...
func assertExactReceiverMethodSets(t *testing.T) {
	...
	withoutMethods := []reflect.Type{
		...
		reflect.TypeOf(FindingCode("")), reflect.TypeOf(Observation{}),
