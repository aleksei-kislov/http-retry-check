...
// started by the client to stop before it reports cleanup as verified. Once
// cleanup finishes, an ended context is returned with any valid partial result.
// A panic from Do or its response body is re-panicked after the same cleanup.
// Options are applied in order, so the last value for a setting wins. A nil
// option or invalid final settings return ErrInvalidCall without invoking Do.
func Run(parent context.Context, doer Doer, options ...Option) (Result, error) {
	if parent == nil || doer == nil {
		return Result{}, ErrInvalidCall
	}
	suiteOptions, ok := applyOptions(options)
	if !ok {
		return Result{}, ErrInvalidCall
	}
	suiteResult, err := scenariosuite.Run(parent, doer, suiteOptions...)
	contextFailure := errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded)
	if err != nil && !contextFailure {
		return Result{}, mapScenarioSuiteRunError(err)
	}
	if contextFailure && suiteResult.Scenarios == nil {
		return Result{}, err
	}
