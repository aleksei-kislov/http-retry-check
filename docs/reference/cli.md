...
`v0.2.0` publishes these executables with a `SHA256SUMS` file on the
[GitHub release](https://github.com/aleksei-kislov/http-retry-check/releases/tag/v0.2.0):
...
Verify the downloaded file against `SHA256SUMS`. On macOS or Linux, make the
binary executable before running it. You can also run the command from a source
checkout with Go 1.25 or later:
...
contains `README.md` and `HttpRetryTests.cs`. Move the files into a module or
test project that references this source, then replace the example client with
your production configuration. The language guides describe run settings and
an optional proxy-free HTTP/1 baseline. A scaffold is not an installer or
standalone project.
...
| `http-retry-check internal failure` (3) | Reading, writing, cleanup, or console output failed after the command started. An input the process cannot read because of permissions, or a destination it cannot write, exits 3. This is not an evidence result. | Check storage and permissions. If it repeats, report the revision, command, exit code, and a reproduction with private data removed. |
