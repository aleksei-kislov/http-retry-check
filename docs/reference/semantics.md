...
| `disconnect_before_acceptance` | Read one request, then close while acceptance remains unconfirmed. | Exactly one observed attempt; no replay and no effect. | A later attempt despite uncertain acceptance, changed request properties, or too many attempts. |
...
| `cross_origin_redirect_credentials` | Return a redirect from one controlled origin to a second. | The client may refuse the redirect, or follow it without forwarding the synthetic credential when the selected attempt limit permits two attempts. | Credential exposure at the target, changed request properties, too many attempts, or more than one target effect. |
| `retry_limit` | Return complete retryable responses. | Between one and the selected attempt limit, with each response complete. | More attempts than the selected limit. |
...
Go options and C# `ScenarioOptions` configure the same four settings:

| Setting | Go | C# | Default | Valid value |
| --- | --- | --- | --- | --- |
| Scenario timeout | `WithScenarioTimeout` | `ScenarioTimeout` | 5 seconds | More than zero, at most 1 minute |
| Connection timeout | `WithConnectionTimeout` | `ConnectionTimeout` | 2 seconds | More than zero, at most 1 minute |
| Quiet window | `WithQuietWindow` | `QuietWindow` | 300 milliseconds | More than zero, at most 1 minute |
| Attempt limit | `WithAttemptLimit` | `AttemptLimit` | 2 | Integer from 1 through 3 |

The scenario timeout covers one complete scenario. The connection timeout
bounds one accepted connection. The quiet window begins after the client call
and its request-body activity finish and observes retries started in the
background. Invalid settings are rejected before the client runs. Go applies
options in order, so the last value for a setting wins.

The attempt limit is recorded in every scenario row. Validation adds
`attempt_limit_exceeded` when the recorded attempt count is greater than that
row's limit. The origin can retain one attempt beyond the limit so the excess
is evidence rather than an unobserved guess. Raising the limit does not make a
replay after acceptance, uncertain acceptance, or a pending response safe;
those scenario-specific rules still apply.
...
- An error or exception in the redirect or retry-limit scenario is accepted
  when every observed attempt and required response is complete. Zero-attempt
  or inconsistent evidence remains incomplete and stops later invocations.
- In these rules, an error means a returned Go error or a C#
  `HttpRequestException`. Other Go panics and unexpected C# client exceptions
  are re-raised after cleanup and do not become evidence.
- After the client invocation and its request bodies finish, the current origin
  stays open for a short, bounded quiet window. A background retry observed in
  that window contributes to the same scenario. Activity that begins later is
  outside the result.
...
Each origin records at most the selected attempt limit plus one and refuses any
later connection. This can stop the run, but findings already recorded, such as
`attempt_limit_exceeded`, remain in the result. A client that retries both
transport failures and status responses may therefore stop before the dedicated
...
- the selected attempt limit and captured protocol;
...
Opening a connection is not by itself an HTTP retry. The suite records retry
counters only after it parses a request head; an admitted connection that sends
no request head instead leaves capture incomplete.

Every row records its `attempt_limit`, including rows that did not run. The
`protocol` value is `HTTP/1.1` only when every admitted connection on the row
produced a complete request head using that protocol. It is empty otherwise,
including when a connection was admitted but sent no request head; in that
case the capture is incomplete. These fields are present in the language
result objects and canonical JSON.
...
The `disconnect_before_acceptance` and `retry_limit` scenarios never record an
effect. A report that gives either scenario a nonzero effect count is invalid.
...
| `attempt_limit_exceeded` | The client made more than the allowed number of attempts. |
...
bytes, one space, and `HTTP/D.D`, where each `D` is one digit. The parsed
protocol is recorded, and only HTTP/1.1 completes a capture. Header names must
be HTTP tokens; folded or malformed lines are rejected, and field values cannot
contain control bytes other than
...
default allowed maximum of two attempts, while the example client makes one.
...
The report schema uses its v2 identity because `attempt_limit` and `protocol`
are now required fields. The suite, explanation, artifact, and corpus
identities remain v1.
