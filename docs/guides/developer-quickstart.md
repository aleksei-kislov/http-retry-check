...
You need Go 1.26 or later. From the repository root:
...
Go 1.26 is the minimum; repository CI uses Go 1.26.6. A passing test logs six
...
Keep its explicit HTTP/1 and proxy settings, replace the example `*http.Client`,
and follow the [complete Go usage guide](go.md) to add the module and save
evidence.
...
project and call `ScenarioTest.CheckAsync` from your own `net10.0` test.
