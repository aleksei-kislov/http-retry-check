...
func TestRunOptionsValidateFinalSettings(t *testing.T) {
	tests := []struct {
		name    string
		options []Option
		valid   bool
		...
		{name: "minimums", options: []Option{
			WithScenarioTimeout(time.Nanosecond), WithConnectionTimeout(time.Nanosecond),
			WithQuietWindow(time.Nanosecond), WithAttemptLimit(1),
		}, valid: true},
		{name: "maximums", options: []Option{
			WithScenarioTimeout(time.Minute), WithConnectionTimeout(time.Minute),
			WithQuietWindow(time.Minute), WithAttemptLimit(3),
		}, valid: true},
		...
		{name: "last duration wins", options: []Option{
			WithScenarioTimeout(0), WithScenarioTimeout(time.Second),
		}, valid: true},
		{name: "last attempt wins", options: []Option{
			WithAttemptLimit(4), WithAttemptLimit(3),
		}, valid: true},
		{name: "last invalid wins", options: []Option{
			WithQuietWindow(time.Second), WithQuietWindow(0),
		}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			configured, valid := applyOptions(productionDependencies(), test.options)
			if valid != test.valid {
				t.Fatalf("valid = %v, want %v (%#v)", valid, test.valid, configured)
			}
			if valid && configured.cleanupTimeout != defaultCleanupTimeout {
				t.Fatal("run options changed the fixed cleanup timeout")
			}
		})
	}
	defaults, valid := applyOptions(productionDependencies(), nil)
	if !valid || defaults.caseTimeout != 5*time.Second ||
		defaults.connectionTimeout != 2*time.Second || defaults.quietWindow != 300*time.Millisecond ||
		defaults.cleanupTimeout != 2*time.Second || defaults.attemptLimit != 2 {
		t.Fatalf("default options = %#v, valid=%v", defaults, valid)
	}
}
...
func TestRunPropagatesClientPanicAfterClosingEveryListenerAndSealingBodies(t *testing.T) {
	tracker := newListenerTracker(0, 0)
	var calls atomic.Uint32
	sentinel := &panicTestMarker{value: 1}
	var replay func() (io.ReadCloser, error)
	recovered, panicked := capturePanic(func() {
		_, _ = run(context.Background(), doerFunc(func(request *http.Request) (*http.Response, error) {
			calls.Add(1)
	...
	if !panicked || recovered != sentinel || calls.Load() != 1 {
		t.Fatalf("recovered = %#v/%t, calls=%d", recovered, panicked, calls.Load())
	}
	if replay == nil {
		t.Fatal("client did not receive replay function")
	}
	if body, err := replay(); body != nil || err != invocationFailure {
		t.Fatalf("replay after panic = %#v/%v", body, err)
	}
	tracker.assertAllClosed(t)
}
...
func TestRunReturnsCancellationDuringSecondScenarioWithValidPartialResult(t *testing.T) {
	tracker := newListenerTracker(0, 0)
	...
	result, err := run(parent, doerFunc(func(request *http.Request) (*http.Response, error) {
		if calls.Add(1) == 2 {
			cancel()
			_ = request.Body.Close()
	...
	if err != parent.Err() || err != context.Canceled || calls.Load() != 2 ||
		...
		t.Fatalf("cancelled run = %#v/%v, calls=%d", result, err, calls.Load())
	}
	tracker.assertAllClosed(t)
}

func TestRunCancellationDuringQuietWindowReturnsValidPartialResult(t *testing.T) {
	tracker := newListenerTracker(0, 0)
	dependency := trackedDependencies(tracker)
	dependency.quietWindow = time.Second
	protocols := new(http.Protocols)
	protocols.SetHTTP1(true)
	transport := &http.Transport{Proxy: nil, Protocols: protocols, DisableKeepAlives: true}
	defer transport.CloseIdleConnections()
	...
	if err != context.Canceled || calls.Load() != 1 || Validate(result) != nil ||
		len(result.Scenarios) != 6 || result.Assessment != AssessmentInconclusive ||
		result.Scenarios[0].Assessment != AssessmentInconclusive ||
		result.Scenarios[0].Observation.CaptureComplete {
		t.Fatalf("quiet-window cancellation = %#v/%v, calls=%d", result, err, calls.Load())
	}
	tracker.assertAllClosed(t)
}

func TestRetryAfterQuietWindowIsOutsideObservation(t *testing.T) {
	listener, err := (&net.ListenConfig{}).Listen(
		context.Background(), "tcp4", "127.0.0.1:0",
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	defer transport.CloseIdleConnections()
	client := &http.Client{Transport: transport}
	dependency := productionDependencies()
	dependency.quietWindow = 10 * time.Millisecond
	var target string
	...
	observation, settled := executeScenario(
		...
		admittedScenario{
			scenario: ScenarioAcceptThenDisconnect, listeners: []net.Listener{listener},
		},
		dependency,
	)
	row, valid := newScenarioResult(ScenarioAcceptThenDisconnect, observation)
	if !settled || !valid || row.Assessment != AssessmentNoUnsafeBehaviorObserved ||
		observation.AttemptCount != 1 {
		t.Fatalf("quiet-window observation = %#v, settled=%v valid=%v", row, settled, valid)
	}

	retry, err := http.NewRequestWithContext(
		context.Background(), http.MethodPost, target, strings.NewReader(syntheticBodyText),
	)
	if err != nil {
		t.Fatal(err)
	}
	retry.Header = header
	retry.ContentLength = int64(len(syntheticBodyText))
	response, retryErr := client.Do(retry)
	if response != nil {
		_ = response.Body.Close()
	}
	if retryErr == nil {
		t.Fatal("retry after the quiet window reached the closed scenario origin")
	}
}
...
func TestResponseBodyClosePanicPropagatesAfterCleanup(t *testing.T) {
	tracker := newListenerTracker(0, 0)
	sentinel := &panicTestMarker{value: 2}
	recovered, panicked := capturePanic(func() {
		_, _ = run(context.Background(), doerFunc(func(request *http.Request) (*http.Response, error) {
			_ = request.Body.Close()
			return &http.Response{
				Body: &faultResponseBody{mode: responseClosePanic, panicValue: sentinel},
			}, nil
		}), trackedDependencies(tracker))
	})
	if !panicked || recovered != sentinel {
		t.Fatalf("response close panic = %#v/%t", recovered, panicked)
	}
	tracker.assertAllClosed(t)
}
...
func TestTypedNilPanicPropagatesAfterCleanup(t *testing.T) {
	tracker := newListenerTracker(0, 0)
	var doer *typedNilDoer
	recovered, panicked := capturePanic(func() {
		_, _ = run(context.Background(), doer, trackedDependencies(tracker))
	})
	if !panicked || recovered != typedNilPanicMarker {
		t.Fatalf("typed-nil panic = %#v/%t", recovered, panicked)
	}
	tracker.assertAllClosed(t)
}

func TestCleanupFailureOrPanicDoesNotReplaceClientPanic(t *testing.T) {
	tests := []struct {
		name    string
		tracker *listenerTracker
	}{
		{name: "close failure", tracker: newListenerTracker(0, 2)},
		{name: "current listener close panic", tracker: newPanicListenerTracker(1)},
		{name: "remaining listener close panic", tracker: newPanicListenerTracker(2)},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			sentinel := &panicTestMarker{value: 3}
			recovered, panicked := capturePanic(func() {
				_, _ = run(context.Background(), doerFunc(func(request *http.Request) (*http.Response, error) {
					_ = request.Body.Close()
					panic(sentinel)
				}), trackedDependencies(test.tracker))
			})
			if !panicked || recovered != sentinel {
				t.Fatalf("cleanup replaced client panic: %#v/%t", recovered, panicked)
			}
			test.tracker.assertAllClosed(t)
		})
	}
}
...
func TestValidateRejectsContradictoryShapesWithoutMutation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "reordered findings", edit: func(result *Result) {
			row := unavailableRow(ScenarioAcceptThenDisconnect, CleanupFailed, 2)
		...
		{name: "attempt limit zero", edit: func(result *Result) { result.Scenarios[0].Observation.AttemptLimit = 0 }},
		{name: "attempt limit above maximum", edit: func(result *Result) { result.Scenarios[0].Observation.AttemptLimit = 4 }},
		{name: "unknown protocol", edit: func(result *Result) { result.Scenarios[0].Observation.Protocol = "HTTP/2" }},
		{name: "complete protocol missing", edit: func(result *Result) { result.Scenarios[0].Observation.Protocol = "" }},
		{name: "complete without attempt", edit: func(result *Result) {
			row := &result.Scenarios[0]
			row.Observation = unavailableObservation(CleanupSucceeded, 2)
			row.Observation.CaptureComplete = true
		}},
...
func TestRetryLimitCountsZeroThroughFour(t *testing.T) {
	for attempts := 0; attempts <= 4; attempts++ {
		t.Run(decimalString(attempts), func(t *testing.T) {
			...
			for index := 0; index < attempts && index < 3; index++ {
				exchangeRawRequest(
					t, origin.endpoints[0].address,
					rawRequest(origin.endpoints[0].address, "HTTP/1.1", []string{syntheticCredential},
						syntheticBodyText, len(syntheticBodyText)),
...
func TestAttemptLimitThreeAdmitsFourthAsValidUnsafeEvidence(t *testing.T) {
	origin, caseContext, cancelCase := startRawOriginWithAttemptLimit(
		t, ScenarioRetryLimit, 3, func(context.Context) bool { return true },
	)
	for range 4 {
		exchangeRawRequest(
			t, origin.endpoints[0].address,
	...
	if !ok || row.Assessment != AssessmentUnsafeBehaviorObserved ||
		observation.AttemptCount != 4 || observation.AttemptLimit != 3 ||
		observation.Protocol != "HTTP/1.1" ||
		!containsFinding(row.Findings, FindingAttemptLimitExceeded) {
		t.Fatalf("limit-three row = %#v, valid=%v", row, ok)
	}
}

func TestObservedProtocolIsEmptyForUnsupportedAndMixedHeads(t *testing.T) {
	tests := []struct {
		name  string
		wires func(*scenarioOrigin) []string
	}{
		{
			name: "unsupported",
			wires: func(origin *scenarioOrigin) []string {
				return []string{rawRequest(origin.endpoints[0].address, "HTTP/1.0",
					[]string{syntheticCredential}, syntheticBodyText, len(syntheticBodyText))}
			},
		},
		{
			name: "mixed",
			wires: func(origin *scenarioOrigin) []string {
				return []string{
					rawRequest(origin.endpoints[0].address, "HTTP/1.1", []string{syntheticCredential},
						syntheticBodyText, len(syntheticBodyText)),
					rawRequest(origin.endpoints[0].address, "HTTP/1.0", []string{syntheticCredential},
						syntheticBodyText, len(syntheticBodyText)),
				}
			},
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for index, wire := range test.wires(origin) {
				if index == 0 && test.name == "mixed" {
					exchangeRawRequest(t, origin.endpoints[0].address, wire)
					continue
				}
				sendRawAndClose(t, origin.endpoints[0].address, wire)
			}
			wantAttempts := 1
			if test.name == "mixed" {
				wantAttempts = 2
			}
			waitForAttemptCount(t, origin, wantAttempts)
			waitForOriginState(t, origin, func(origin *scenarioOrigin) bool {
				return len(origin.connections) == 0
			})
			observation := finishRawOrigin(t, origin, caseContext, cancelCase)
			row, ok := newScenarioResult(ScenarioRetryLimit, observation)
			if !ok || row.Assessment != AssessmentInconclusive || observation.Protocol != "" ||
				observation.CaptureComplete {
				t.Fatalf("protocol row = %#v, valid=%v", row, ok)
			}
		})
	}
}

func TestObservedProtocolIsEmptyWhenAdmittedConnectionSendsNoHead(t *testing.T) {
	origin, caseContext, cancelCase := startRawOrigin(
		t, ScenarioRetryLimit, func(context.Context) bool { return true },
	)
	exchangeRawRequest(
	...
	bareConnection := dialRaw(t, origin.endpoints[0].address)
	if err := bareConnection.Close(); err != nil {
		t.Fatal(err)
	}
	...
	row, ok := newScenarioResult(ScenarioRetryLimit, observation)
	if !ok || row.Assessment != AssessmentInconclusive || observation.Protocol != "" ||
		observation.CaptureComplete || observation.AttemptCount != 2 {
		t.Fatalf("head-plus-bare-admission row = %#v, valid=%v", row, ok)
	}
}
...
func TestBlockedResponseWriteUsesScenarioSpecificOverlapPhase(t *testing.T) {
	tests := []struct {
		...
		{name: "delayed response write included", scenario: ScenarioDelayedResponse,
			wantOverlap: 1, wantActiveOnBlocked: 2},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func TestDelayedPendingPhaseClearsBeforeLaterAdmission(t *testing.T) {
	for _, mode := range []string{"completed", "interrupted"} {
		t.Run(mode, func(t *testing.T) {
			...
			retryBefore := origin.retryBeforeResponse
			origin.mu.Unlock()
			if overlap != 0 || retryAfter != 0 || retryBefore != 0 {
...
func TestDelayedFailedResponseEndsBeforeLaterBareAdmission(t *testing.T) {
	origin := &scenarioOrigin{
		scenario: ScenarioDelayedResponse, attempts: []capturedAttempt{{complete: true}},
		attemptLimit: 2, connections: make(map[net.Conn]bool), active: 1, effectCount: 1,
		pendingResponse: 1,
	}
	server, client := net.Pipe()
	_ = client.Close()
	...
	laterServer, laterClient := net.Pipe()
	_, admitted := origin.admit(laterServer, sourceEndpoint)
	if !admitted || origin.retryAfterEffect != 0 || origin.retryBeforeResponse != 0 {
		t.Fatalf("later bare-admission counters = after:%d before:%d",
			origin.retryAfterEffect, origin.retryBeforeResponse)
	}
	_ = laterClient.Close()
	_ = laterServer.Close()
	origin.mu.Lock()
	delete(origin.connections, laterServer)
	if origin.active != 0 {
		origin.active--
	}
	origin.mu.Unlock()
...
func TestResultsAndFixedErrorsDoNotLeakCallerMarkers(t *testing.T) {
	...
	for _, mode := range []sanitationMode{
		sanitationErrorMode, sanitationFieldsMode,
	} {
	...
	unavailableDependency := dependencies{
		...
		attemptLimit: 2, delay: func(context.Context) bool { return true },
	}
...
func startRawOrigin(
	...
	return startRawOriginWithAttemptLimit(t, scenario, 2, delay)
}

func startRawOriginWithAttemptLimit(
	t *testing.T,
	scenario ScenarioID,
	attemptLimit uint32,
	delay func(context.Context) bool,
	...
	origin, ok := newScenarioOrigin(scenario, listeners, time.Second, attemptLimit, delay, cancelCase)
	if !ok {
		cancelCase()
...
type panicTestMarker struct {
	value uint8
}

func capturePanic(operation func()) (value any, panicked bool) {
	defer func() {
		value = recover()
		panicked = value != nil
	}()
...
var typedNilPanicMarker = &panicTestMarker{value: 4}

func (doer *typedNilDoer) Do(request *http.Request) (*http.Response, error) {
	_ = request.Body.Close()
	if doer == nil {
		panic(typedNilPanicMarker)
	}
	return nil, testFailure(1)
}
...
func newPanicListenerTracker(closePanicAt int) *listenerTracker {
	return &listenerTracker{closePanicAt: closePanicAt}
}
...
func (tracker *listenerTracker) listen(
	...
	tracked := &trackedListener{
		Listener: listener, closeFailure: call == tracker.closeFailureAt,
		closePanic: call == tracker.closePanicAt,
	}
	tracker.mu.Lock()
...
func (listener *trackedListener) Close() error {
	...
	if listener.closePanic {
		panic(&panicTestMarker{value: 5})
	}
