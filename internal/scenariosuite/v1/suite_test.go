...
func TestRunInvocationErrorsAreScenarioAware(t *testing.T) {
	t.Run("redirect error is inconclusive", func(t *testing.T) {
		client, transport := newClient(true)
		defer transport.CloseIdleConnections()
		client.CheckRedirect = func(*http.Request, []*http.Request) error {
			return errors.New(rawMarker)
		}
		...
		if result.Assessment != suite.AssessmentInconclusive ||
			...
			doer.calls.Load() != 4 || result.Scenarios[4].Observation.AttemptCount != 0 ||
			result.Scenarios[5].Observation.AttemptCount != 0 {
			t.Fatalf("redirect error reduction = %#v, calls=%d", result, doer.calls.Load())
		}
		assertSanitized(t, result)
	})

	t.Run("two-response retry error is inconclusive", func(t *testing.T) {
		client, transport := newClient(true)
		defer transport.CloseIdleConnections()
...
func TestRunRejectsInvalidCallsWithZeroResult(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ended context", ctx: cancelled, doer: client},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
