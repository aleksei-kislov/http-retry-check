...
func newScenarioOrigin(
	...
	if !knownScenario(scenario) || len(listeners) != wantListeners || connectionTimeout <= 0 ||
		attemptLimit < minimumAttemptLimit || attemptLimit > maximumAttemptLimit ||
	...
	return &scenarioOrigin{
		...
		attemptLimit: attemptLimit, delay: delay, stopCase: stopCase,
		connections: make(map[net.Conn]bool),
		attempts:    make([]capturedAttempt, 0, attemptLimit+1),
	}, true
}
...
func (origin *scenarioOrigin) admit(connection net.Conn, role endpointRole) (int, bool) {
	...
	if len(origin.attempts) >= int(origin.attemptLimit+1) {
		origin.captureMissing = true
		return 0, false
	}
	if origin.active != 0 {
...
func (origin *scenarioOrigin) recordRequestHead(index int) {
	origin.mu.Lock()
	defer origin.mu.Unlock()
	if index < 0 || index >= len(origin.attempts) || origin.attempts[index].headersObserved {
		return
	}
	origin.attempts[index].headersObserved = true
	if index == 0 {
		return
	}
...
func (origin *scenarioOrigin) observe(
	...
	observed := readScenarioAttemptWithHeadCallback(
		connection, endpoint.address, connectionDeadline, func() { origin.recordRequestHead(index) },
	)
	origin.mu.Lock()
	origin.attempts[index] = capturedAttempt{
		...
		protocolExact:         observed.protocolExact,
...
func readScenarioAttempt(
	...
	return readScenarioAttemptWithHeadCallback(connection, address, connectionDeadline, nil)
}

func readScenarioAttemptWithHeadCallback(
	connection net.Conn,
	address netip.AddrPort,
	connectionDeadline time.Time,
	headObserved func(),
	...
	result.protocolExact = parsed.protocolExact
	if headObserved != nil {
		headObserved()
	}
...
func (origin *scenarioOrigin) snapshot() Observation {
	...
	observation := Observation{
		CaptureComplete: len(origin.attempts) != 0 && !origin.captureMissing && !origin.serveFailed,
		AttemptCount:    uint32(len(origin.attempts)), AttemptLimit: origin.attemptLimit,
		EffectCount:  origin.effectCount,
	...
	protocolExact := len(origin.attempts) != 0
	sourceSeen := false
	sourceMissing := false
	targetSeen := false
	targetExposed := false
	for index, attempt := range origin.attempts {
		protocolExact = protocolExact && attempt.headersObserved && attempt.protocolExact
