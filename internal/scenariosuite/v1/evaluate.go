...
func validObservation(scenario ScenarioID, observation Observation) bool {
	...
	if !knownScenario(scenario) ||
		observation.AttemptLimit < minimumAttemptLimit || observation.AttemptLimit > maximumAttemptLimit ||
		observation.AttemptCount > maxObservedAttempts ||
		observation.AttemptCount > observation.AttemptLimit+1 ||
		!knownProtocol(observation.Protocol) ||
		(observation.AttemptCount == 0 && observation.CaptureComplete) ||
		(observation.AttemptCount == 0 && observation.Protocol != "") ||
		(observation.CaptureComplete && observation.AttemptCount != 0 && observation.Protocol != "HTTP/1.1") ||
		(scenario == ScenarioCrossOriginRedirectCredentials && observation.CaptureComplete &&
			observation.AttemptCount > 2 && observation.DestinationConsistent) ||
...
func assess(scenario ScenarioID, observation Observation) (Assessment, []FindingCode) {
	...
	if exceededAttemptLimit(observation) {
	...
	if effectLimitExceeded(observation.EffectCount) {
...
func exceededAttemptLimit(observation Observation) bool {
	return observation.AttemptCount > observation.AttemptLimit
}
...
func positiveTuple(scenario ScenarioID, observation Observation) bool {
	...
	case ScenarioRetryLimit:
		return observation.AttemptCount >= 1 && observation.AttemptCount <= observation.AttemptLimit &&
