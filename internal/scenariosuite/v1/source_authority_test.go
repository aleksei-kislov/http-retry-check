...
func TestPublicSurfaceMatchesExpectedAPI(t *testing.T) {
	...
	wantTypes := map[string]bool{
		...
		"ScenarioResult": true, "Result": true, "RunError": true, "Option": true,
	}
	wantFunctions := map[string]bool{
		"EvaluateObservation": true, "Run": true, "Validate": true,
	...
	assertStructFields(t, reflect.TypeOf(suite.Observation{}), []string{
		"CaptureComplete", "AttemptCount", "AttemptLimit", "Protocol", "EffectCount", "OverlapCount",
	...
	assertStructFieldTypes(t, reflect.TypeOf(suite.Observation{}), []reflect.Type{
		reflect.TypeOf(false), reflect.TypeOf(uint32(0)), reflect.TypeOf(uint32(0)), reflect.TypeOf(""),
		reflect.TypeOf(uint64(0)), reflect.TypeOf(uint32(0)), reflect.TypeOf(uint32(0)),
	...
	if !runType.IsVariadic() || runType.NumIn() != 3 || runType.In(0) != contextType ||
		runType.In(1) != doerType || runType.In(2) != reflect.TypeOf([]suite.Option{}) ||
	...
	optionType := reflect.TypeOf(suite.Option(nil))
	if optionType.Kind() != reflect.Func || optionType.NumIn() != 1 || optionType.NumOut() != 0 {
		t.Fatalf("Option signature = %v", optionType)
	}
	for name, function := range map[string]any{
		"WithScenarioTimeout":   suite.WithScenarioTimeout,
		"WithConnectionTimeout": suite.WithConnectionTimeout,
		"WithQuietWindow":       suite.WithQuietWindow,
	} {
	...
	attemptOptionType := reflect.TypeOf(suite.WithAttemptLimit)
