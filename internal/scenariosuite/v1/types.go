// Package scenariosuite runs the six local HTTP/1 scenarios against a Go HTTP client.
...
// CleanupState reports whether the suite stopped and cleaned up its listeners,
// connections, handlers, and tracked request bodies.
...
// SourceOnly means it reached the source, but the target result is unknown; it
// does not prove that the client stripped the credential everywhere else.
...
// FindingCode identifies why a scenario did not pass. Codes never contain
// runtime values, addresses, bodies, credentials, responses, or raw errors.
...
// Observation records the facts used to calculate findings and assessments.
// Its counts cover only the suite's test servers, not all traffic from the client.
...
// Findings is always non-nil in a valid result.
...
// RunError identifies a failure returned by Run.
type RunError uint8
...
// Error returns the fixed message for this failure type.
...
// Run executes all six scenarios against doer.
func Run(parent context.Context, doer Doer, options ...Option) (Result, error) {
	dependency, ok := applyOptions(productionDependencies(), options)
	if !ok {
		return Result{}, ErrInvalidCall
	}
	return run(parent, doer, dependency)
}
