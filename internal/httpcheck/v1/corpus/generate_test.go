...
func buildRowBundles(t *testing.T) map[string]rowBundle {
	...
	unsafeRows := []rowCase{
		...
		newRow(t, "unsafe_retry_limit_exceeded", scenarios[4], attemptLimit(withCounts(positiveRetryOne(), 4, 0, 0, 0, 0, 0, 4, 4, true, 0), 3),
...
func buildInvalidBundles() map[string]invalidBundle {
	...
	semantic := []invalidCase{
		...
		invalid("semantic_attempt_limit_zero", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/attempt_limit", "0", 0, "field.attempt_limit.lower"),
		invalid("semantic_attempt_limit_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/attempt_limit", "4", 0, "field.attempt_limit.above_max"),
		invalid("semantic_attempt_limit_missing", "invalid_result", both, "result_all_positive", "remove", "/scenarios/0/observation/attempt_limit", "", 0, "field.attempt_limit.missing"),
		invalid("semantic_protocol_missing", "invalid_result", both, "result_all_positive", "remove", "/scenarios/0/observation/protocol", "", 0, "field.protocol.missing"),
		invalid("semantic_protocol_unsupported", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/protocol", `"HTTP/2"`, 0, "field.protocol.unsupported"),
		invalid("semantic_protocol_missing_for_complete_capture", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/protocol", `""`, 0, "relationship.protocol_capture"),
		invalid("semantic_attempt_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/attempt_count", "5", 0, "field.attempt_count.above_max"),
		invalid("semantic_attempt_above_recorded_limit_ceiling", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/attempt_count", "4", 0, "relationship.attempt_limit"),
		invalid("semantic_effect_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/effect_count", "5", 0, "field.effect_count.above_max"),
		invalid("semantic_overlap_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/overlap_count", "4", 0, "field.overlap_count.above_max"),
		invalid("semantic_retry_after_effect_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/0/observation/retry_after_effect_count", "4", 0, "field.retry_after_effect_count.above_max"),
		invalid("semantic_retry_after_unconfirmed_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/1/observation/retry_after_unconfirmed_count", "4", 0, "field.retry_after_unconfirmed_count.above_max"),
		invalid("semantic_retry_before_response_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/5/observation/retry_before_response_count", "4", 0, "field.retry_before_response_count.above_max"),
		invalid("semantic_response_attempt_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/3/observation/response_attempt_count", "5", 0, "field.response_attempt_count.above_max"),
		invalid("semantic_response_complete_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/3/observation/response_complete_count", "5", 0, "field.response_complete_count.above_max"),
		invalid("semantic_delay_above_max", "invalid_result", both, "result_all_positive", "set", "/scenarios/5/observation/delay_complete_count", "5", 0, "field.delay_complete_count.above_max"),
	...
	reportModel := []invalidCase{
		...
		invalid("report_schema_drift", "invalid_report", both, "positive", "set", "/schema_version", `"http_retry_check.report.v1"`, 0, "report.identity.schema", "report.identity.previous"),
	...
	canonicalJSON := []invalidCase{
		...
		invalid("json_wrong_identity", "invalid_report", both, "positive", "set", "/schema_version", `"http_retry_check.report.v1"`, 0, "json.identity", "report.identity.previous"),
	...
	negativeTargets := []string{
		...
		"/scenarios/0/observation/attempt_count", "/scenarios/0/observation/attempt_limit", "/scenarios/0/observation/effect_count",
	...
	sanitation := []invalidCase{
		...
		invalid("sanitation_panic_value", "panic_propagated", []string{"go"}, "", "inject_marker", "panic_value", markerValue, 0, "sanitation.panic"),
...
func positiveAccept() observation {
	return observation{CaptureComplete: true, AttemptCount: 1, AttemptLimit: 2, Protocol: "HTTP/1.1", EffectCount: 1, MethodConsistent: true,
...
func positiveDisconnect() observation {
	return observation{CaptureComplete: true, AttemptCount: 1, AttemptLimit: 2, Protocol: "HTTP/1.1", MethodConsistent: true,
		DestinationConsistent: true, BodyConsistent: true, Credential: "source_only", Cleanup: "succeeded"}
}
...
func positiveRedirectRefused() observation {
	return observation{CaptureComplete: true, AttemptCount: 1, AttemptLimit: 2, Protocol: "HTTP/1.1", ResponseAttemptCount: 1,
		ResponseCompleteCount: 1, FirstResponseComplete: true, MethodConsistent: true,
		DestinationConsistent: true, BodyConsistent: true, Credential: "source_only", Cleanup: "succeeded"}
}

func positiveRedirectFollowed() observation {
	return observation{CaptureComplete: true, AttemptCount: 2, AttemptLimit: 2, Protocol: "HTTP/1.1", EffectCount: 1, ResponseAttemptCount: 2,
...
func attemptLimit(value observation, limit uint32) observation {
	value.AttemptLimit = limit
	return value
}
...
func acceptMaxCounts() observation {
	value := attemptLimit(positiveAccept(), 3)
	value.AttemptCount, value.EffectCount, value.RetryAfterEffectCount = 4, 4, 3
	return value
}

func disconnectMaxCounts() observation {
	value := attemptLimit(positiveDisconnect(), 3)
	value.AttemptCount, value.RetryAfterUnconfirmedCount = 4, 3
	return value
}

func delayedMaxCounts() observation {
	value := attemptLimit(positiveDelayedComplete(), 3)
	value.CaptureComplete = false
	value.AttemptCount, value.EffectCount, value.RetryAfterEffectCount = 4, 4, 3
	value.RetryBeforeResponseCount = 3
	value.ResponseAttemptCount, value.ResponseCompleteCount = 4, 4
	value.DelayCompleteCount = 4
	return value
}

func retryOverlapMax() observation {
	value := attemptLimit(positiveRetryOne(), 3)
	value.CaptureComplete = false
	value.AttemptCount, value.OverlapCount = 4, 3
	value.ResponseAttemptCount, value.ResponseCompleteCount = 4, 4
	return value
}

func unrun(cleanupState string) observation {
	return observation{AttemptLimit: 2, MethodConsistent: true, DestinationConsistent: true, BodyConsistent: true,
