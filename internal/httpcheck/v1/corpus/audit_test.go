...
func TestScenarioIncompleteCannotBeProducedAndForgeryIsRejected(t *testing.T) {
	...
	want := map[string]uint64{
		"accept_then_disconnect":            10182,
		"disconnect_before_acceptance":      4038,
		"changed_body_retry":                10182,
		"cross_origin_redirect_credentials": 71606,
		"retry_limit":                       17382,
		"delayed_response":                  403814,
	}
...
func TestObservationFieldsHaveBoundsAndContradictionControls(t *testing.T) {
	...
	wantMaximum := map[string]uint64{
		"attempt_count": 4, "effect_count": 4, "overlap_count": 3,
		"retry_after_effect_count": 3, "retry_after_unconfirmed_count": 3,
		"retry_before_response_count": 3, "response_attempt_count": 4,
		"response_complete_count": 4, "delay_complete_count": 4,
	}
...
func TestInvalidAndNativeControlInventory(t *testing.T) {
	...
	wantCounts := map[string]int{"semantic": 71, "report_model": 20, "canonical_json": 35,
...
func closedInvalidGrammar(category string, candidate invalidCase) bool {
	...
	expected := map[string]map[string]bool{
		...
		"sanitation":     {"marker_absent": true, "panic_propagated": true},
...
func TestFixtureIdentityExpectedClassAndRepresentability(t *testing.T) {
	...
	const want = "4edbd8c02459fcfa7d6f84e382e75b1ad4728a4f1f478e3f465e921606827e6b"
	if got != want {
...
func visitBoundedObservations(visit func(observation)) {
	...
	for attemptLimit := uint32(1); attemptLimit <= 3; attemptLimit++ {
		for attempts := uint32(0); attempts <= attemptLimit+1; attempts++ {
			later := uint32(0)
			if attempts != 0 {
				later = attempts - 1
			}
														...
														for _, cleanup := range cleanups {
															for capture := 0; capture < 2; capture++ {
																...
																for _, protocol := range protocols {
																	if (attempts == 0 && protocol != "") || (captureComplete && protocol != "HTTP/1.1") {
																		continue
																	}
																	visit(observation{CaptureComplete: captureComplete, AttemptCount: attempts,
																		AttemptLimit: attemptLimit, Protocol: protocol,
