...
// This file is a test-only, language-neutral restatement of the frozen v1
...
func validObservation(scenario string, value observation) bool {
	...
	if !knownScenario(scenario) || value.AttemptCount > 3 ||
...
func assess(scenario string, value observation) (string, []string) {
	...
	if effectLimitExceeded(scenario, value.EffectCount) {
...
func effectLimitExceeded(scenario string, effects uint64) bool {
	if scenario == scenarios[1] || scenario == scenarios[4] {
		return effects != 0
	}
	return effects > 1
}
...
func positiveTuple(scenario string, value observation) bool {
	...
	case scenarios[4]:
		return value.AttemptCount >= 1 && value.AttemptCount <= 2 && value.EffectCount == 0 &&
