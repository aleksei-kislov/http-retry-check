...
type atomicRenameNoReplaceFunc func(parent *os.File, oldName, newName string) error

type atomicNoReplaceProbeStatus uint8

const (
	atomicNoReplaceProbeFailed atomicNoReplaceProbeStatus = iota
	atomicNoReplaceProbeUnsupported
	atomicNoReplaceProbeSupported
)

// WriteArtifact writes a complete artifact atomically and never replaces an
// existing path.
func WriteArtifact(files []evidence.ArtifactFile, destination string) error {
	return writeArtifact(files, destination, atomicRenameNoReplace)
}

func writeArtifact(files []evidence.ArtifactFile, destination string, renameNoReplace atomicRenameNoReplaceFunc) (err error) {
	...
	if err != nil || !stableStaging(walk, &stage) || !exactCreatedInventory(stage.root, stage.files) || !walk.stable() {
		return ErrInternalFailure
	}
	probeStatus := classifyAtomicNoReplaceProbe(renameNoReplace(parentHandle, stagingLeaf, stagingLeaf))
	...
	if probeStatus != atomicNoReplaceProbeSupported || !stableStaging(walk, &stage) ||
		!exactCreatedInventory(stage.root, stage.files) || !walk.stable() {
		return ErrInternalFailure
	}
	// Recheck the final name before populating the stage. A target that appeared
	// during stage setup is reported as unavailable only after the still-empty
	// owned stage is removed with its exact identity intact.
	finalAbsent, finalAbsenceErr = walk.absent(walk.leaf)
	...
	if renameErr := renameNoReplace(parentHandle, stagingLeaf, walk.leaf); renameErr != nil {
