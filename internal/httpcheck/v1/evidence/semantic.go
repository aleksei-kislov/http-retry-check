...
const (
	...
	minimumAttemptLimit = 1
	maximumAttemptLimit = 3
...
func validObservation(scenario string, observation Observation) bool {
	...
	if !knownScenario(scenario) || observation.AttemptCount > maxObservedAttempts ||
		...
		(observation.Protocol != "" && observation.Protocol != http11Protocol) ||
		(observation.AttemptCount == 0 && observation.Protocol != "") ||
		(observation.CaptureComplete && observation.Protocol != http11Protocol) ||
...
func findingText(finding string) string {
	...
	case findingAttemptLimitExceeded:
		return "The client made more than the allowed number of attempts."
