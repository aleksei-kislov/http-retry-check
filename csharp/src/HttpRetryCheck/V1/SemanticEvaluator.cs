...
internal static class SemanticEvaluator
{
    ...
    private static bool IsValidObservation(ScenarioId scenario, Observation observation)
    {
        ...
        if (!IsKnownScenario(scenario) ||
            observation.AttemptCount > Runtime.RuntimeConstants.MaximumObservedAttempts ||
            observation.AttemptLimit is < Runtime.RuntimeConstants.MinimumAttemptLimit or
                > Runtime.RuntimeConstants.MaximumAttemptLimit ||
            observation.AttemptCount > observation.AttemptLimit + 1 ||
            observation.Protocol is null ||
            (observation.Protocol.Length != 0 &&
                !observation.Protocol.Equals(Runtime.RuntimeConstants.Http11Protocol, StringComparison.Ordinal)) ||
            (observation.AttemptCount == 0 && observation.Protocol.Length != 0) ||
            (observation.CaptureComplete &&
                !observation.Protocol.Equals(Runtime.RuntimeConstants.Http11Protocol, StringComparison.Ordinal)) ||
            (scenario == ScenarioId.CrossOriginRedirectCredentials && observation.CaptureComplete &&
        ...
        Observation observation)
    {
        ...
        if (observation.AttemptCount > observation.AttemptLimit)
        {
    ...
    private static bool EffectLimitExceeded(ulong effects) => effects > 1;
    ...
    private static bool IsPositiveTuple(ScenarioId scenario, Observation observation)
    {
        ...
        switch (scenario)
        {
            ...
            case ScenarioId.RetryLimit:
                return observation.AttemptCount >= 1 &&
                    observation.AttemptCount <= observation.AttemptLimit &&
