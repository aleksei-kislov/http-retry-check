...
internal sealed class ScenarioOrigin
{
    ...
    private readonly uint attemptLimit;
    private readonly int maximumObservedAttempts;
    ...
    private readonly List<CapturedAttempt> attempts;
    private readonly List<Task> acceptTasks = new(2);
    private readonly List<Task> handlerTasks;
        ...
        Func<TimeSpan, CancellationToken, Task>? waitForDelayedResponse = null)
    {
        ...
        this.maximumObservedAttempts = maximumObservedAttempts;
        this.waitForDelayedResponse = waitForDelayedResponse ?? Task.Delay;
        attempts = new List<CapturedAttempt>(maximumObservedAttempts);
        handlerTasks = new List<Task>(maximumObservedAttempts);
    }
    ...
    internal Observation Snapshot()
    {
        lock (sync)
        {
            ...
            var protocolExact = attempts.Count != 0;
            ...
            return new Observation(
                !captureMissing && !serveFailed && protocolExact,
                (uint)attempts.Count,
                attemptLimit,
                protocolExact ? RuntimeConstants.Http11Protocol : string.Empty,
                effectCount,
                overlapCount,
    ...
    private int Admit(Socket connection, EndpointRole role)
    {
        lock (sync)
        {
            if (closing || attempts.Count >= maximumObservedAttempts)
            {
                if (attempts.Count >= maximumObservedAttempts)
                {
                    captureMissing = true;
                }
    ...
    private void RecordRequestHead(int index)
    {
        lock (sync)
        {
            if (index == 0)
    ...
    private sealed class CapturedAttempt
    {
        ...
        internal bool ProtocolExact { get; private set; }
        ...
        internal void Apply(CaptureResult result)
        {
            ...
            ProtocolExact = result.ProtocolExact;
