...
public sealed class PublicSurfaceTests
{
    ...
    public void AssemblyExportsOnlyTheDocumentedPublicTypes()
    {
        ...
        [
            ...
            "HttpRetryCheck.V1.ScenarioOptions",
    ...
    public void RootMembersMatchTheExpectedApi()
    {
        AssertMembers<Observation>(
            "ctor(bool,uint,uint,string,ulong,uint,uint,uint,uint,uint,uint,bool,uint,bool,bool,bool,CredentialState,CleanupState)",
            "property uint AttemptLimit",
        ...
        AssertMembers(typeof(ScenarioSuite),
            ...
            "static Task<SuiteResult> RunAsync(HttpMessageInvoker,ScenarioOptions,CancellationToken)",
            "static void Validate(SuiteResult)");
        AssertMembers<ScenarioOptions>(
            "ctor(Nullable<TimeSpan>,Nullable<TimeSpan>,Nullable<TimeSpan>,uint)",
            "property uint AttemptLimit",
            "property TimeSpan ConnectionTimeout",
            "property TimeSpan QuietWindow",
            "property TimeSpan ScenarioTimeout");
        ...
        AssertSealedClass(typeof(ScenarioOptions));
        AssertSealedClass(typeof(ScenarioResult));
        AssertSealedClass(typeof(SuiteResult));
        AssertSealedClass(typeof(SuiteException));

        AssertCancellationDefault(typeof(ScenarioSuite), nameof(ScenarioSuite.RunAsync), 1, 2);
        AssertCancellationDefault(typeof(ScenarioSuite), nameof(ScenarioSuite.RunAsync), 2, 3);

        ConstructorInfo optionsConstructor = typeof(ScenarioOptions).GetConstructors().Single();
        ParameterInfo[] optionParameters = optionsConstructor.GetParameters();
        CollectionAssert.AreEqual(
            new[] { "scenarioTimeout", "connectionTimeout", "quietWindow", "attemptLimit" },
            optionParameters.Select(static parameter => parameter.Name).ToArray());
        Assert.IsTrue(optionParameters.Take(3).All(static parameter =>
            parameter.HasDefaultValue && parameter.DefaultValue is null));
        Assert.IsTrue(optionParameters[3].HasDefaultValue);
        Assert.AreEqual((uint)2, optionParameters[3].DefaultValue);
    }
    ...
    public void TestingMembersMatchTheExpectedApi()
    {
        ...
        AssertMembers(typeof(ScenarioTest),
            ...
            "static Task CheckAsync(HttpMessageInvoker,ScenarioOptions,Action<string>,CancellationToken)");
        ...
        MethodInfo method = typeof(ScenarioTest).GetMethod(
            ...
            binder: null,
            [typeof(HttpMessageInvoker), typeof(Action<string>), typeof(CancellationToken)],
            modifiers: null)
        ...
        MethodInfo configured = typeof(ScenarioTest).GetMethod(
            nameof(ScenarioTest.CheckAsync),
            BindingFlags.Public | BindingFlags.Static,
            binder: null,
            [typeof(HttpMessageInvoker), typeof(ScenarioOptions), typeof(Action<string>), typeof(CancellationToken)],
            modifiers: null)
            ?? throw new AssertFailedException("Configured CheckAsync is missing.");
        Assert.IsTrue(configured.GetParameters()[2].HasDefaultValue);
        Assert.IsNull(configured.GetParameters()[2].DefaultValue);
        AssertCancellationDefault(typeof(ScenarioTest), nameof(ScenarioTest.CheckAsync), 2, 3);
        AssertCancellationDefault(typeof(ScenarioTest), nameof(ScenarioTest.CheckAsync), 3, 4);
    }
    ...
    public void ReportingMembersMatchTheExpectedApi()
    {
        ...
        AssertMembers(typeof(ScenarioReports),
            ...
            "const string SchemaId=urn:http-retry-check:schema:report:v2",
            "const string SchemaVersion=http_retry_check.report.v2",
    ...
    public void NullableAnnotationsMatchTheExpectedApi()
    {
        ...
        MethodInfo run = typeof(ScenarioSuite).GetMethod(
            nameof(ScenarioSuite.RunAsync),
            [typeof(HttpMessageInvoker), typeof(CancellationToken)])
        ...
        MethodInfo check = typeof(ScenarioTest).GetMethod(
            nameof(ScenarioTest.CheckAsync),
            [typeof(HttpMessageInvoker), typeof(Action<string>), typeof(CancellationToken)])
            ?? throw new AssertFailedException("CheckAsync is missing.");
    ...
    private static void AssertCancellationDefault(
        Type owner,
        string methodName,
        int parameterIndex,
        int parameterCount)
