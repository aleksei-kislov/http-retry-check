...
public sealed class ReportingAuthorityTests
{
    ...
    public void OutcomeAndConstantsMatchTheExpectedApi()
    {
        ...
        Assert.AreEqual("http_retry_check.report.v1", constants[nameof(ScenarioReports.SchemaVersion)]);
        Assert.AreEqual(
            "urn:http-retry-check:schema:report:v1",
    ...
    public void ReportingMembersAndConstructorsMatchTheExpectedApi()
    {
        ...
        AssertConstructor<ReportObservation>(
            typeof(bool), typeof(uint), typeof(ulong), typeof(uint), typeof(uint), typeof(uint), typeof(uint),
