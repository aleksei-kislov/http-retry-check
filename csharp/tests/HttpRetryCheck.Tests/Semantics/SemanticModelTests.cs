...
public sealed class SemanticModelTests
{
    ...
    [TestMethod]
    public void CountAboveRecordedLimitIsValidUnsafeEvidence()
    {
        var observation = new Observation(
            true, 2, 1, "HTTP/1.1", 0, 0, 0, 0, 0, 2, 2, true, 0,
            true, true, true, CredentialState.SourceOnly, CleanupState.Succeeded);
        var row = new ScenarioResult(
            ScenarioId.RetryLimit,
            Assessment.UnsafeBehaviorObserved,
            observation,
            new List<FindingCode> { FindingCode.AttemptLimitExceeded });

        Assert.AreEqual((uint)2, row.Observation.AttemptCount);
        Assert.AreEqual((uint)1, row.Observation.AttemptLimit);
        CollectionAssert.AreEqual(
            new[] { FindingCode.AttemptLimitExceeded },
            Copy(row.Findings));
    }
    ...
    public void SuiteValidationRecomputesUnsafeOverInconclusiveAggregate()
    {
        ...
        rows[3] = new ScenarioResult(
            ...
            Assessment.UnsafeBehaviorObserved,
            new Observation(
                true, 2, 2, "HTTP/1.1", 1, 0, 0, 0, 0, 2, 2, true, 0,
    ...
    public void ConstructorsRejectInvalidValuesOnlyWithInvalidResult()
    {
        AssertInvalid(() => new Observation(
            true, 5, 2, "HTTP/1.1", 1, 0, 0, 0, 0, 0, 0, false, 0,
            true, true, true, CredentialState.SourceOnly, CleanupState.Succeeded));
        AssertInvalid(() => new Observation(
            false, 0, 2, string.Empty, 0, 0, 0, 0, 0, 0, 0, false, 0,
        ...
        AssertInvalid(() => new Observation(
            true, 1, 0, "HTTP/1.1", 1, 0, 0, 0, 0, 0, 0, false, 0,
            true, true, true, CredentialState.SourceOnly, CleanupState.Succeeded));
        AssertInvalid(() => new Observation(
            true, 1, 4, "HTTP/1.1", 1, 0, 0, 0, 0, 0, 0, false, 0,
            true, true, true, CredentialState.SourceOnly, CleanupState.Succeeded));
        AssertInvalid(() => new Observation(
            true, 1, 2, "HTTP/2", 1, 0, 0, 0, 0, 0, 0, false, 0,
            true, true, true, CredentialState.SourceOnly, CleanupState.Succeeded));
        AssertInvalid(() => new Observation(
            true, 1, 2, null!, 1, 0, 0, 0, 0, 0, 0, false, 0,
            true, true, true, CredentialState.SourceOnly, CleanupState.Succeeded));
        AssertInvalid(() => new Observation(
            false, 0, 2, "HTTP/1.1", 0, 0, 0, 0, 0, 0, 0, false, 0,
            true, true, true, CredentialState.NotObserved, CleanupState.Succeeded));
        AssertInvalid(() => new Observation(
            true, 3, 3, "HTTP/1.1", 1, 0, 0, 0, 0, 3, 3, true, 0,
            true, true, true, CredentialState.AbsentAtTarget, CleanupState.Succeeded));
